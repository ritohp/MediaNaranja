  ...
  const exportPDF = async () => {
    ...
    try {
        ...
        const canvas = await html2canvas(area, { 
            ...
            logging: false,
            // Eliminamos dimensiones explícitas para evitar el fallo del iframe
        });
        ...
        alert(`Error al generar: "${err.message}". Prueba refrescando o usando fotos más pequeñas.`);
    } finally {
      ...
      <div className="max-w-[1500px] mx-auto px-6 grid grid-cols-1 xl:grid-cols-12 gap-10">
        ...
        <div className="xl:col-span-4 space-y-6">
          <div className="bg-[#111] p-8 rounded-[2.5rem] border border-white/5 space-y-8 shadow-2xl">
            <header className="border-b border-white/5 pb-6">
                ...
                <p className="text-gray-500 text-[10px] uppercase tracking-[0.3em] mt-2">Personalización Completa</p>
            </header>

            <section className="space-y-4">
              <h3 className="text-[10px] font-black uppercase text-[#E50914]">1. Portada del Recuerdo</h3>
            ...
            {/* SECCIÓN RESTAURADA DE FOTOS CHICAS */}
            <section className="space-y-4">
              <h3 className="text-[10px] font-black uppercase text-[#E50914]">2. Momentos Galería</h3>
              <div className="grid grid-cols-5 gap-2">
                {galleryPhotos.map((photo, i) => (
                  <div key={i} onClick={() => document.getElementById(`gallery-${i}`)?.click()} className="aspect-square bg-[#0a0a0a] rounded-lg border border-gray-800 flex items-center justify-center cursor-pointer hover:border-[#E50914] overflow-hidden">
                    {photo ? <img src={photo} className="w-full h-full object-cover" /> : <Camera size={14} className="text-gray-700 font-bold" />}
            ...
            <button onClick={exportPDF} disabled={isExporting} className={`w-full py-6 bg-white text-black rounded-full font-black text-xs uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-3 shadow-2xl ${isExporting ? 'opacity-50' : 'hover:bg-red-50'}`}>
              {isExporting ? <><Loader2 className="animate-spin" size={20} /> Preparando PDF...</> : <><Download size={20} /> DESCARGAR DISEÑO </>}
            </button>
          </div>
        </div>

        {/* ÁREA DE CAPTURA (ESTABLE) */}
        <div className="xl:col-span-8 flex justify-center sticky top-24">
          ...
          >
            ...
            {/* HEADER */}
            <header className="absolute top-0 inset-x-0 h-14 flex items-center justify-between px-10 z-50 bg-black/90">
            ...
            <div className="absolute inset-x-10 bottom-44 z-40 space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white/90 font-bold tracking-[0.4em] text-[7px] uppercase underline decoration-[#E50914] underline-offset-4">Película Exclusiva</span>
                </div>
